...
namespace SWARM.Server.Controllers.Applications
{
    ...
    public class ZipController : BaseController, iBaseController<Zipcode>
    {
        ...
        public async Task<IActionResult> Post([FromBody] Zipcode _Item)
        {
            ...
            try
            {
                var exists = await _context.Zipcodes.Where(x => x.Zip == _Item.Zip).FirstOrDefaultAsync();
                if (exists != null)
                {
                ...
                exists = new Zipcode();
                exists.Zip = _Item.Zip;
                _context.Zipcodes.Add(exists);
                await _context.SaveChangesAsync();
                trans.Commit();
                return Ok(_Item.Zip);
            }
        ...
        public async Task<IActionResult> Put([FromBody] Zipcode _Item)
        {
            ...
            try
            {
                ...
                exists = new Zipcode();
                exists.Zip = _Item.Zip;
                _context.Zipcodes.Update(exists);
                await _context.SaveChangesAsync();
                trans.Commit();
                return Ok(_Item.Zip);
            }
